...
public class CoachDashboardController {
    ...
    @FXML private VBox coachDashboardPane;
    @FXML private VBox athletesPane;
    @FXML private VBox chatroomPane;
    @FXML private VBox workoutCatalogPane;
    @FXML private VBox feedbackPane;
    @FXML private VBox mentalWellnessPane;
    @FXML private VBox nutritionPane;
    @FXML private VBox profilePane;
    @FXML private Canvas objectiveCanvas;
    @FXML private VBox objectiveLegendBox;
    @FXML private Label totalClientsLabel;
    @FXML private Button coachHomeBtn;
    @FXML private Button athletesBtn;
    @FXML private Button chatroomBtn;
    @FXML private Button workoutCatalogBtn;
    @FXML private Button feedbackBtn;
    @FXML private Button mentalWellnessBtn;
    @FXML private Button nutritionBtn;
    @FXML private Button profileBtn;
    @FXML private PieChart objectiveChart;
    @FXML private Label navbarPageTitle;
    @FXML private Label navbarPageSubtitle;
    @FXML private Button navbarBellBtn;
    @FXML private Label navbarUserName;
    @FXML private Label navbarUserRole;
    @FXML private Label navbarAvatar;
    ...
    private void setNavbarText(String title, String subtitle) {
        if (navbarPageTitle != null) navbarPageTitle.setText(title);
        if (navbarPageSubtitle != null) navbarPageSubtitle.setText(subtitle);
    }

    private void refreshNavbar() {
        if (navbarBellBtn != null) navbarBellBtn.setText("\uD83D\uDD14");
        User session = AppSession.getCurrentUser();
        if (session == null) return;
        String initials = userInitials(session);
        if (navbarUserName != null) navbarUserName.setText(initials);
        if (navbarAvatar != null) navbarAvatar.setText(initials);
        if (navbarUserRole != null) navbarUserRole.setText("Coach");
    }
    ...
    private static String userInitials(User u) {
        ...
        if (!email.isEmpty()) return email.substring(0, Math.min(2, email.length())).toUpperCase();
        return "CH";
    }
    ...
    private void setupAthletesTable() {
        ...
        nameCol.setCellFactory(col -> new TableCell<>() {
            ...
            { fullNameLabel.getStyleClass().add("user-name"); emailLabel.getStyleClass().add("user-email"); wrapper.getStyleClass().add("user-cell"); }
        ...
        roleCol.setCellFactory(col -> new TableCell<>() {
            ...
            @Override protected void updateItem(String role, boolean empty) {
                ...
                roleBadge.setText(role); setGraphic(roleBadge);
            }
        });
        ...
        statusCol.setCellFactory(col -> new TableCell<>() {
            ...
            @Override protected void updateItem(String status, boolean empty) {
                ...
                statusBadge.setText(status.toUpperCase()); setGraphic(statusBadge);
            }
        });
    }
    ...
    private void refreshStats() {
        ...
        long active = roster.stream().filter(u -> "active".equalsIgnoreCase(safe(u.getAccountStatus()))).count();
        ...
        try {
            ...
            double avg = workouts.stream().filter(w -> w.getDuree() != null)
                    .mapToInt(Workout::getDuree).average().orElse(0);
            ...
            for (Workout w : workouts) {
                for (Exercise e : w.getExercises()) {
                    exerciseUsage.merge(e.getNom() != null ? e.getNom() : "Unknown", 1, Integer::sum);
                }
            }
            List<Map.Entry<String, Integer>> top = exerciseUsage.entrySet().stream()
                    .sorted(Map.Entry.<String, Integer>comparingByValue().reversed()).limit(8).toList();
            ...
            if (exerciseBarChart != null) {
                ...
                javafx.application.Platform.runLater(() -> {
                    ...
                    int i = 0;
                    for (javafx.scene.Node bar : exerciseBarChart.lookupAll(".chart-bar")) {
                        bar.setStyle("-fx-bar-fill:" + colors[i % colors.length] + ";"); i++;
                    }
                });
            }
            ...
            for (Workout w : workouts) {
                if (w.getObjectifs().isEmpty()) { goalDist.merge("Unknown", 1, Integer::sum); }
                else { for (var obj : w.getObjectifs()) goalDist.merge(obj.getName() != null ? obj.getName() : "Unknown", 1, Integer::sum); }
            }
            if (goalPieChart != null) {
                goalPieChart.getData().clear();
                goalDist.forEach((k, v) -> goalPieChart.getData().add(new PieChart.Data(k + " (" + v + ")", v)));
            ...
            for (Workout w : workouts) {
                levelDist.merge(w.getNiveau() != null ? capitalize(w.getNiveau()) : "Unknown", 1, Integer::sum);
            }
            if (levelPieChart != null) {
                levelPieChart.getData().clear();
                levelDist.forEach((k, v) -> levelPieChart.getData().add(new PieChart.Data(k + " (" + v + ")", v)));
        ...
        // Feedback stats (placeholder — populate when feedback service is connected)
        if (totalFeedbacksLabel != null) totalFeedbacksLabel.setText("—");
        if (positiveSentimentLabel != null) positiveSentimentLabel.setText("—");

        // Rating chart — BarChart showing rating distribution 1-5
        if (ratingChartContainer != null) {
            ratingChartContainer.getChildren().clear();
            javafx.scene.chart.BarChart<String, Number> ratingChart =
                    new javafx.scene.chart.BarChart<>(
                            new javafx.scene.chart.CategoryAxis(),
                            new javafx.scene.chart.NumberAxis());
            ratingChart.setStyle("-fx-background-color:transparent;");
            ratingChart.setLegendVisible(false);
            ratingChart.setPrefHeight(180);
            XYChart.Series<String, Number> ratingSeries = new XYChart.Series<>();
            ratingSeries.getData().add(new XYChart.Data<>("⭐", 0));
            ratingSeries.getData().add(new XYChart.Data<>("⭐⭐", 0));
            ratingSeries.getData().add(new XYChart.Data<>("⭐⭐⭐", 0));
            ratingSeries.getData().add(new XYChart.Data<>("⭐⭐⭐⭐", 0));
            ratingSeries.getData().add(new XYChart.Data<>("⭐⭐⭐⭐⭐", 0));
            ratingChart.getData().add(ratingSeries);
            ratingChartContainer.getChildren().add(ratingChart);
        }

        // Sentiment chart — PieChart
        if (sentimentChartContainer != null) {
            sentimentChartContainer.getChildren().clear();
            javafx.scene.chart.PieChart sentimentChart = new javafx.scene.chart.PieChart();
            sentimentChart.setStyle("-fx-background-color:transparent;");
            sentimentChart.setLabelsVisible(false);
            sentimentChart.setLegendVisible(true);
            sentimentChart.setPrefHeight(180);
            sentimentChart.getData().add(new PieChart.Data("Positive (0)", 1));
            sentimentChart.getData().add(new PieChart.Data("Neutral (0)", 1));
            sentimentChart.getData().add(new PieChart.Data("Negative (0)", 1));
            sentimentChartContainer.getChildren().add(sentimentChart);
        }
    }
    ...
    private void refreshAthletesTable() {
        try { athletesTable.setItems(FXCollections.observableArrayList(loadRosterUsers())); }
        catch (Exception e) { athletesTable.setItems(FXCollections.observableArrayList()); }
    }
    ...
    private void switchScene(String fxmlPath, String cssPath) {
        ...
        } catch (IOException e) { throw new RuntimeException("Failed to switch scene", e); }
    }
}
